...
form.addEventListener("submit", function (e) {
  ...
  let textamount = input.value;
  if (textamount < 0 ) {
   alert (`Could Not Generate ${input.value} Paragraphs Please Input A Valid Number`)
  }else if (textamount=='') {
      alert('Input Field Is Empty')
  }else{
      getarray(input.value);
  }
  //   if (textamount <= 0 || textamount == "" || textamount > text.length) {
  //     const random = Math.floor(Math.random() *8);

  //     textamount = random+1;
  //     input.value = random+1;
  //   } else {
